        ...
        <header class="bg-slate-950 text-white">
            ...
            <section class="mx-auto max-w-7xl px-6 pb-10 pt-6">
                <div class="rounded-lg bg-blue-600 p-6 shadow-2xl shadow-blue-950/30 sm:p-8">
                    <h1 class="text-3xl font-bold sm:text-4xl">Edit Kategori</h1>
                    <p class="mt-3 max-w-2xl text-sm leading-6 text-blue-100 sm:text-base">Perbarui informasi kategori <strong>{{ $category->name }}</strong>.</p>
                </div>
            </section>
        </header>
            ...
            <section class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
                <form method="POST" action="{{ route('categories.update', $category) }}">
                    ...
                    <div class="mb-5">
                        ...
                        <input id="name" type="text" name="name" value="{{ old('name', $category->name) }}" class="w-full rounded-lg border border-slate-200 px-4 py-3 text-sm text-slate-950 transition focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="Masukkan nama kategori">
                        @error('name')
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="description" name="description" rows="4" class="w-full rounded-lg border border-slate-200 px-4 py-3 text-sm text-slate-950 transition focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="Masukkan deskripsi kategori (opsional)">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
