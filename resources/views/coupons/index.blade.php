...
@section('body')
        <x-admin.sidebar active="coupons" action-label="Generate Report" action-icon="assessment" />

        <x-admin.header placeholder="Cari kupon..." show-notifications show-admin-label />
            ...
            <div class="mx-auto max-w-[1280px] p-margin-mobile md:p-margin-desktop">
                <div class="mb-8 flex flex-col justify-between gap-6 md:flex-row md:items-center">
                    <div>
                        ...
                        <p class="mt-1 font-body-md text-body-md text-on-surface-variant">Kelola kupon diskon, kode promo, dan parameter sistem global.</p>
                    </div>
                    <a href="{{ route('coupons.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 font-label-md text-label-md font-bold text-on-primary shadow-lg shadow-primary/10 transition-all hover:scale-[1.02] hover:shadow-primary/20 active:scale-95">
                            ...
                            <tbody class="divide-y divide-outline-variant">
                                @foreach ($coupons as $coupon)
                                    <tr class="transition-colors hover:bg-surface-container-lowest">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="font-body-md text-body-md font-bold text-secondary">{{ $coupon->discount_percentage }}%</span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex items-center justify-end gap-1">
                                                <a href="{{ route('coupons.edit', $coupon) }}" class="rounded-lg p-2 text-primary transition-colors hover:bg-surface-container" aria-label="Edit {{ $coupon->code }}">
                                ...
                                @if ($coupons->isEmpty())
                                    <tr>
                                        <td colspan="6" class="px-6 py-12 text-center font-label-md text-label-md text-on-surface-variant">Belum ada kupon yang ditambahkan.</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
